...
namespace Examples.Tutorials
{
    ...
    /// </summary>
    [Example("Example 6.2: Diffuse Reflection", ExampleCategory.Learning, "6. Lighting", 1, Source = "Tutorial06_2", Documentation = "Tutorial06_2")]
    public class TutorialLearning06_2 : GameWindow
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            uNMatrix = p.getUniformMatrix3f("uNMatrix");

            uPMatrix = p.getUniformMatrix4f("uPMatrix");

            Uniform3f uLightPosition = p.getUniform3f("uLightPosition");
            Uniform3f uKd = p.getUniform3f("uKd");
            Uniform3f uLd = p.getUniform3f("uLd");
            uLightPosition.set( new Vector3f( 2.0f, 2.0f, 2.0f));
            uKd.set(new Vector3f(0.2f, 0.8f, 0.0f));
            uLd.set(new Vector3f(0.7f, 0.7f, 0.7f));
        ...
        {
                Matrix3f normalMatrix = ((Matrix3f)Matrix4f.Invert(viewMatrix));
        ...
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            mat =  Matrix4f.CreateTranslation(0.5f, 3.0f, 0.0f) * Matrix4f.CreateRotation(angle * 3, 0.0f, 1.0f, 0.5f);
            SetNormalMatrix(mat);
            uMVMatrix.set(camera.ViewMatrix * mat);
            fb.draw(p, mesh1);

            mat = Matrix4f.CreateTranslation(5.0f, 3.0f, 0.0f) * Matrix4f.CreateRotation(angle * 3, 0.0f, 1.0f, 0.5f);
            SetNormalMatrix(mat);
            uMVMatrix.set(camera.ViewMatrix * mat);
            fb.draw(p, mesh2);
