...
namespace Examples.Tutorials
{
    ...
    /// </summary>
    [Example("Example 6.2: GDI Texture Formats", ExampleCategory.Testing, "6. Textures", 1, Source = "Tutorial06_2", Documentation = "Tutorial-TODO")]
    public class Tutorial06_2 : GameWindow
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            // For a complete suite of BMP images, see https://github.com/jsummers/bmpsuite
            Bitmap texture1 = new Bitmap("Resources/BmpSuite/rgb32.bmp");
            Bitmap texture2 = new Bitmap("Resources/BmpSuite/rgb24.bmp");
            Bitmap texture3 = new Bitmap("Resources/BmpSuite/rgb16.bmp");
            Bitmap texture4 = new Bitmap("Resources/BmpSuite/rgb16-565.bmp");
            t1 = CreateTexture(texture1);
            t2 = CreateTexture(texture2);
            t3 = CreateTexture(texture3);
            t4 = CreateTexture(texture4);
            quad = new Mesh<Vertex16_V3T2f, uint>(Vertex16_V3T2f.SizeInBytes, sizeof(uint), MeshMode.TRIANGLES, MeshUsage.GPU_STATIC, 4, 6);
            quad.addAttributeType(0, 3, AttributeType.A32F, false);
            quad.addAttributeType(1, 2, AttributeType.A32F, false);
            quad.addVertex(new Vertex16_V3T2f() { Position = new Vector3f(-0.9f, -0.9f, 0), TexCoord = new Vector2f(0, 0) });
            quad.addVertex(new Vertex16_V3T2f() { Position = new Vector3f(-0.1f, -0.9f, 0), TexCoord = new Vector2f(1, 0) });
            quad.addVertex(new Vertex16_V3T2f() { Position = new Vector3f(-0.9f, -0.1f, 0), TexCoord = new Vector2f(0, 1) });
            quad.addVertex(new Vertex16_V3T2f() { Position = new Vector3f(-0.1f, -0.1f, 0), TexCoord = new Vector2f(1, 1) });
            quad.addIndice(0);
            quad.addIndice(1);
            quad.addIndice(2);
            quad.addIndice(2);
        ...
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            p.getUniformSampler("gSampler").set(t4);
            translation.set(new Vector3f(1, 1, 0));
            fb.draw(p, m, MeshMode.TRIANGLES, 0, 6);
            this.SwapBuffers();
        }
        ...
        FrameBuffer fb;
        Program p;
        Mesh<Vertex16_V3T2f, uint> quad;
        MeshBuffers m;
        Texture t1, t2, t3, t4;
        GPUBuffer[] buff = new GPUBuffer[4];
...
@"#ifdef _VERTEX_
        ...
        {
            using (Tutorial06_2 example = new Tutorial06_2())
            {
