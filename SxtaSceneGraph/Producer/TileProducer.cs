...
namespace Sxta.Render.Producer
{
    ...
    public class TileProducer
    {
        ...
        {
            int users = 0;
            TileCache.Tile t = cache.getTile(id, level, tx, ty, deadline, ref users);
            if (users == 0)
            {
        ...
        public bool updateTileMap(float splitDistance, Vector2f camera, int maxLevel)
        {
            ...
            tileMapT.setSubImage(0, 0, id, n, 1, TextureFormat.RG, PixelType.UNSIGNED_BYTE, new Buffer.Parameters(), new CPUBuffer<byte>(tileMap));
            return true;
        }
        ...
        protected virtual void swap(TileProducer p)
        {
            cache.invalidateTiles(id);
        ...
        /*
          ...
          */
        internal virtual object getContext()
        {
            return null;
        }
        ...
        /*
          ...
          */
        internal virtual Task startCreateTile(int level, int tx, int ty, uint deadline, Task task, TaskGraph owner)
        {
            for (int i = 0; i < (int)layers.Count; i++)
            {
        ...
        /*
          ...
          */
        internal virtual bool doCreateTile(int level, int tx, int ty, TileStorage.Slot data)
        {
            bool changes = false;
            for (int i = 0; i < (int)layers.Count; i++)
            {
        ...
        /*
          ...
          * each Layer of this %producer.
          */
        internal virtual void endCreateTile()
        {
            for (int i = 0; i < (int)layers.Count; i++)
            {
        ...
        /*
          ...
          * @param ty the tile's quadtree y coordinate.
          */
        internal virtual void stopCreateTile(int level, int tx, int ty)
        {
            for (int i = 0; i < (int)layers.Count; i++)
            {
        ...
        {
            CreateTile t = task as CreateTile;
            Debug.Assert(t != null);
        ...
        internal Task createTile(int level, int tx, int ty, TileStorage.Slot data, uint deadline, Task old)
        {
            ...
            if (old != null)
            {
                CreateTileTaskGraph r = old as CreateTileTaskGraph;
                if (r != null)
                {
                    r.restore();
                }
                ...
                {
                    Debug.Assert(old is CreateTile);
                }
                return old;
            }
            CreateTile t = new CreateTile(this, level, tx, ty, data, deadline);
            Task rt = startCreateTile(level, tx, ty, deadline, t, null);
            lock (mutex)
            {
                tasks.Add(t);
                if (rt.Equals(t))
                {
                    Debug.Assert(rt is CreateTileTaskGraph);
                    tasks.Add(rt);
                }
     ...
     */
    internal class CreateTile : Task
    {
        ...
        public void start()
        {
            if (!initialized)
            {
                ...
                if (parent != null)
                {
                    ...
                    foreach (var t in parent.getLastTasks())
                    {
                        if (t != this)
                        {
                            parent.removeTask(t);
        ...
        public override void setIsDone(bool done, uint t, reason r)
        {
            ...
            else if (r == reason.DATA_NEEDED)
            {
                ...
                TileCache.Tile tt = owner.findTile(level, tx, ty, true);
     ...
     * of type CreateTile.
     */
    internal class CreateTileTaskGraph : TaskGraph
    {
         ...
         */
        public ISet<Task> rootDependencies = new HashSet<Task>();
